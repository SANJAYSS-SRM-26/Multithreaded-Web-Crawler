...
class MultiThreadedCrawler:
    ...
    def __init__(self, seed_url, max_pages=50, output_file='output.txt'):
    ...
    def save_to_file(self, url, title, description, text):
    ...
    def post_scrape_callback(self, res):
        ...
        if result and result.status_code == 200:
            ...
            meta_description = soup.find("meta", attrs={"name": "description"})
            description = meta_description.get("content") if meta_description else "No description found"
            text = self.scrape_info(result.text)
            self.save_to_file(self.current_scraping_url, title, description, text)
...
if __name__ == '__main__':
    ...
    cc = MultiThreadedCrawler(user_input_url, max_pages_to_crawl, output_file)
